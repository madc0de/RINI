 ...
 * From a DSN (data source name):
   ...
   ```php
   $db = \Rini\Core\Db\PdoDatabase::fromDsn(
       new \Rini\Core\Db\PdoDsn(
 ...
 * Using an existing `PDO` instance:
   ...
   $db = \Rini\Core\Db\PdoDatabase::fromPdo($pdo);
   ```
   ...
   ```php
   $db = \Rini\Core\Db\PdoDatabase::fromPdo($pdo, true);
   ```

#### Available drivers for database systems

```php
\Rini\Core\Db\PdoDataSource::DRIVER_NAME_MYSQL;
\Rini\Core\Db\PdoDataSource::DRIVER_NAME_POSTGRESQL;
\Rini\Core\Db\PdoDataSource::DRIVER_NAME_SQLITE;
```
...
```php
$db->setProfiler(new \Rini\Core\Db\SimpleProfiler());
```
...
```php
$db->addOnConnectListener(function (\Rini\Core\Db\PdoDatabase $db) {
	// do something
});
```
